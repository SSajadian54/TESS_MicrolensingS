...
const int nm=int(23855);//12:24282;//  
///=============================================================================
struct source{
    int    nums,struc,cl;
...
int main()
{
   ...
   double effe, nwei, fte,ddo, nwea, meter,par0, par1, par2, par3, par4; 
   double lonn, countd, tei, tE, totn, wei,eff0, eff1, eff2, eff3, eff4, deltaA;   
...
///=============================================================================    
   ...
   if(!magdi){cout<<"cannot read sec13_parB.txt "<<sector<<endl; exit(0);}
   for(int i=0; i<nm; ++i){//i,tmag,fb,teff,radius,metal,logg,ra, dec, los
   fscanf(magdi,"%d  %lf  %lf  %lf  %lf  %lf  %lf  %lf  %lf  %lf\n",
   &numm, &t.Map[i], &t.blend[i], &t.teff[i], &t.radius[i], &t.mh[i], &t.logg[i], &t.ra[i], &t.dec[i], &ddo);
   //cout<<"t_Map:  "<<t.Map[i]<<endl; 
    //  int uue; cin>>uue; 
   }
   fclose(magdi); 
...
///=============================================================================
   ...
   if(s.magb>=d.satu and s.magb<=d.detect and fte<s.blend){
   nsim+=1;
   ...
   "%d  %d   %.5lf  %.5lf  "///4
   "%d  %.8lf   %.6lf   %.5lf  "///8
   "%d   %d  %.5lf   %.5lf  %.4lf  %.4lf  %.4lf  %.2lf  %.3lf  %.4lf  %.4lf  "   //21
   "%.5lf  %.8lf    %.2lf   %.4lf   "  //29
   "%.6lf  %.5lf  %.5lf  %.7lf  %.5lf  %.5lf  %.4lf  %.7lf  %.9lf  " ///38
   "%d  %.1lf  %d   %d  %.5lf  %.5lf\n", //44
   los, nsim, s.lat, lonn, //4
   l.struc, log10(l.Ml), l.Dl, l.vl, //8
   s.struc, s.cl, s.mass, s.Ds, s.Tstar, s.Rstar, s.logg, s.type, l.vs, s.Mab, s.Map,//19
   s.magb, s.blend, s.nsbl, s.Ai, //23
   l.tE, l.RE/AU, l.t0, l.mul, l.Vt, l.u0, s.opd*1.0e6, s.ros, l.tetE,//32
   detectf,d.dchi,d.det,d.numt,d.timet, meter);}//38
   }while(ndet<50);
   fclose(distr);
...
///HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH   
   ...
   Nevent=gama*co.Nstar[los]*d.Tobs;  
   ...
   cout<<"ddeg:  "<<ddeg<<"\t Ntot:  "<<Ntot<<"\t co.Nstar[los]: "<<co.Nstar[los]<<endl;  
   ...
   log10(d.opta), log10(d.optb), log10(gama), Nevent, Ntot, //18
...
void func_lens(lens & l, source & s)
{
    ...
    do{
    l.numl=int(RandR(0.0,s.nums-1.0));
