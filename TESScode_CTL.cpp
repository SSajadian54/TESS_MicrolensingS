...
const int sector=0;  ///updated 6 Feb
const int camera=1; 
//const int nm[4]={39034, 38860, 30226, 38322};
const int nmmax= 162681;
...
int main()
{
   ...
   double rf, effe,Av, nwei, fte,ddo, nwea, meter,par0, par1, par2, par3, par4; 
   double lonn, countd, tei, tE, totn, wei,eff0, eff1, eff2, eff3, eff4, deltaA;   
   double gama, Nevent, Ntot=0.0;
   double maga, maga2, chi2, chi1,emt, u, As, Astar, timee,dt, tscale; 
   double flag0,flag1,flag2, t1,t2;
   int ID;
   double ra, dec; 
...
///HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
   ...
   for(int bn1=0; bn1<10000; ++bn1){
   
   
   num=int(RandR(0.0,nmmax-1.0));
   sprintf(filenam7,"./files/%c%c%c.dat",'C','T','L');
   magdi=fopen(filenam7,"r"); 
   if(!magdi){cout<<"cannot read CTL.dat "<<sector<<endl; exit(0);}
   for(int i=0; i<num; ++i){
   fscanf(magdi,"%d  %lf  %lf  %lf  %lf  %lf  %lf  %lf  %lf  %lf  %lf  %lf %lf  %lf\n",
   &ID,&ra,&dec, &t.glong, &t.glat, &t.elong, &t.elat, &t.Map, &t.error, &t.teff,&t.radius,&t.dist,&t.blend,&t.prior);
  ...
  // if(s.magb>=d.satu and s.magb<=d.detect and l.tE<=tEmax and l.tE>=tEmin and s.ros<99.0){
   ...
   if(s.ros>1.0) {tscale=l.tE*s.ros;  t1=-2.5*tscale+l.t0; t2=2.5*tscale+l.t0; }
   else          {tscale=l.tE;        t1=-2.5*tscale+l.t0; t2=2.5*tscale+l.t0; }
   if(t1>0.0)     t1=0.0;  
   if(t2<d.Tobs)  t2=d.Tobs;
   ...
   fclose(tempo);}
    ...
    double piE=double(1.0/l.Dl-1.0/s.Ds)/l.tetE; 
   fprintf(distr,
   "%d  %d   %.5lf  %.5lf  "///4
   "%d  %.8lf   %.6lf   %.5lf  "///8
   ...
   s.struc, s.nums, 1.0, s.Ds, s.Tstar, s.Rstar, t.prior, s.type, l.vs, s.Mab, s.Map,//19
   s.magb, s.blend, s.nsbl, s.Ai, //23
   l.tE, l.RE/AU, l.t0, l.mul, l.Vt, l.u0, s.opd*1.0e8, s.ros, l.tetE,//32
   detectf,d.dchi,d.det,d.numt, piE, meter);//38
   }
   }
  // }
