...
const double mmin0=double(0.01*Mearth/M_sun);//in M_sun  low limit for planets

const double ml1=log10(mmin0); 
const double ml2=log10(2.0); 
const double dl1=0.0;
const double dl2=8.0;  
...
const double AlAv[M]= {1.13  ,  0.8558}; //https://tasoc.dk/docs/release_notes/tess_sector_12_drn17_v02.pdf 
const double sigma[M]={0.017 ,  0.02};//// G, T
const int Nes=int(4000);
...
const int    sector =13;  ///updated 6 Feb
...
struct detection{
   ...
   double cadence, detect[2], satu[2], Tobs;  
   double gap1, dur1, gap2, dur2;
   double magn[Nes];   
};
struct CMD{
...
int main()
{
   ...
   d.detect[1]=18.0;
   d.satu[1]=  1.0;
   d.Tobs=     27.942;//days updated 3 Feb  [21/5/2019,   18/6/2019]
   d.gap1=     14.046;//[21/5/2019,  04/6/2019]  OBSERVATION
   d.dur1=     1.0375;//[4/6/2019,  5/6/2019]  GAP
   d.gap2=     20.0;  
   d.dur2=     0.0;} 
   ...
   d.dur2=     1.0403;}//[1/11/2018,   2/11/2018]  GA
   ...
   d.detect[0]=21.0;  d.satu[0]=5.9;//Gaia 
   double tEmin=double(d.cadence*0.1);
   double tEmax=60.0;      
   int    num1, num2; 
   char   filenam0[40],filenam1[40], filenam2[40], filenam3[40], filenam4[40], filenam5[40], filenam6[40];
   int    nsim, ndet, los=0, countb, ffd, v0, v1, v2, v3, v4, detectf, fflag;
   double effe, nwei, fte, nwea, meter,par0, par1, par2, par3, par4; 
   double lonn, countd, tei, tE, wei,eff0, eff1, eff2, eff3, eff4, deltaA;   
   double gama, Nevent,Ntot=0.0;
   double maga, maga2, chi2, chi1,emt, u, As, Astar, timee,dt; 
...
///=============================================================================    
   sprintf(filenam5,"./files/%c%c%c%c%c%d.dat",'C','o','o','r','S',sector);
   coord=fopen(filenam5,"r");
   if(!coord){ cout<<"cannot read CoordS.dat "<<sector<<endl; exit(0);}
   for(int i=0; i<int(Nco*Nco); ++i){
   fscanf(coord,"%d %d %lf %lf %lf %lf %lf %lf\n",&num1,&num2,&co.elat[i],&co.elong[i],&co.b[i],&co.l[i],&co.dec[i],&co.ra[i]);}
   fclose(coord);
  
   
   sprintf(filenam3,"./files/light/lcF/%c%c%c%c%d.dat",'T','E','S','S',sector);
   result=fopen(filenam3,"w");
   fclose(result);  
    
    
   sprintf(filenam4,"./files/light/lcF/%c%c%c%c%c%d.dat",'E','T','E','S','S',sector);
...
///=============================================================================
   ...
   sprintf(filenam0,"./files/light/lcF/%c%d%c%d.dat",'D',sector,'_',los);
   distr=fopen(filenam0,"w"); 
   
   
   sprintf(filenam6,"./files/%c%c%c%d.dat",'T','E','M',sector);
   tempo=fopen(filenam6,"w");///temporary file
   fclose(tempo);  
   ...
   func_source(s,cm,ex);
   func_lens(l,s);
   }while(l.tE>tEmax or l.tE<tEmin or s.ros>99.0); 
   ...
   if(s.magb[1]>=d.satu[1] and s.magb[1]<=d.detect[1] and fte<s.blend[1]){
   nsim+=1;
   s.nddis[s.nums]+=1.0*d.weight;
   d.opta +=s.opd*1.0e6*d.weight;  
   nwei   +=1.0*d.weight; 
   d.Nsim +=1.0*d.weight;

    
        
   for(int di=0; di<Nes; di++) d.magn[di]=0.0;  
   dt=double(l.tE/200.0);
   if(dt>d.cadence) dt=double(d.cadence/3.0); 
   ...
   if(nsim<3 and los%50==0){
   ffd=1;
   sprintf(filenam1,"./files/light/lcF/%c%d%c%d%c%d.dat",'L',sector,'_',los,'_',nsim);
   fild=fopen(filenam1,"w");
   sprintf(filenam2,"./files/light/lcF/%c%d%c%d%c%d.dat",'M',sector,'_',los,'_',nsim);
   film=fopen(filenam2,"w");}
  
 
  
   for(d.t=-2.0; d.t<=d.Tobs+2.0; d.t=d.t+dt){
   u=sqrt(l.u0*l.u0+(d.t-l.t0)*(d.t-l.t0)/l.tE/l.tE);
   As=vbb.ESPLMag2(u, s.ros); 
   Astar=double(As*s.blend[1]+1.0-s.blend[1]);   
   maga=s.magb[1]-2.5*log10(Astar);
   if(ffd>0)    fprintf(film,"%.4lf    %.4lf   %.4lf\n",d.t, maga, Astar);
   if(d.t>0 and d.t<=d.Tobs and float((d.t-d.gap1)*(d.t-d.gap1-d.dur1))>=0.0 and float((d.t-d.gap2)*(d.t-d.gap2-d.dur2))>=0.0){
   timee+=dt; 
   if(timee>=d.cadence){
   timee-=d.cadence; 
   if(maga>=d.satu[1] and maga<=d.detect[1]){
   emt= fabs( ErrorTESS(d,maga) );
   deltaA=fabs(pow(10.0,-0.4*emt)-1.0)*Astar; 
   maga2=maga+RandN(emt, 3.0);
   d.error+= emt; 
   d.magn[d.numt]=maga2; 
   if(Astar<1.05)   countb+=1;      
   chi1 += pow((maga2-s.magb[1])/emt,2.0);
   chi2 += pow((maga2-     maga)/emt,2.0);
   if(d.det==0){
   flag2=0.0;
   if( float(s.magb[1]-maga2)>float(5.0*emt))     flag2=1.0;
   ...
   if(countb<10) d.fbase=0.0;
   else          d.fbase=1.0;  
   if(fabs(l.t0)<float(2.0*d.cadence) or fabs(d.Tobs-l.t0)<float(2.0*d.cadence))  d.fside=1.0;
   else            d.fside=0.0;    
   ...
   if(d.dchi>=500.0 and d.det>0){//Lensing detection
   detectf=  1;  
   ndet  +=  1;
   nwea  +=  1.0*d.weight; 
   d.Ndet+=  1.0*d.weight;      
   d.optb+=  s.opd*1.0e6*d.weight;  
   d.tE  +=  l.tE*d.weight; 
   d.fb  +=  s.blend[1]*d.weight; 
   d.mbase+= s.magb[1]*d.weight; 
   d.Dl  +=  l.Dl*d.weight; 
   d.Ds  +=  s.Ds*d.weight; 
   ...
   v4=int(TEdet(d,s.magb[1], d.satu[1], d.detect[1]) );
   ...
   "%d  %d   %.5lf  %.5lf  "///4
   "%d  %.8lf   %.6lf   %.5lf  "///8
   "%d   %d  %.5lf   %.5lf  %.4lf  %.4lf  %.4lf  %.2lf  %.3lf  %.4lf  %.4lf   %.4lf   %.4lf   "   //21
   "%.5lf  %.5lf  %.8lf  %.8lf  %.2lf  %.2lf   %.4lf  %.4lf   "  //29
   "%.6lf  %.5lf  %.5lf  %.7lf  %.5lf  %.5lf  %.4lf  %.7lf  %.9lf  " ///38
   "%d  %.1lf  %d   %d  %.5lf  %.5lf\n", //44
   los, nsim, s.lat, lonn, //4
   l.struc, log10(l.Ml), l.Dl, l.vl, //8
   s.struc, s.cl, s.mass, s.Ds, s.Tstar, s.Rstar, s.logl, s.type, l.vs, s.Mab[0], s.Mab[1], s.Map[0], s.Map[1],//21
   s.magb[0],s.magb[1], s.blend[0], s.blend[1], s.nsbl[0], s.nsbl[1], s.Ai[0], s.Ai[1], //29
   l.tE, l.RE/AU, l.t0, l.mul, l.Vt, l.u0, s.opd*1.0e6, s.ros, l.tetE,//38
   detectf,d.dchi,d.det,d.numt,d.timet, meter);}//44
   }while(ndet<10);
   fclose(distr);
...
///HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH   
   ...
   par4=double(d.satu[1]+(d.detect[1]-d.satu[1])*j/GG/1.00);  
   ...
   gama=2.0*d.opta*1.0e-6*tei/M_PI;//[N_event/days]
   Nevent=gama*s.nstart*d.Tobs;//N_event(deg^{-2})        
   ...
   fprintf(result,"%d  %.5lf   %.5lf   %.5lf   %.5lf  %.5lf   %.5lf   %d  %d  "//9
   "%.6lf  %.6lf  %.6lf   %.6lf "  //13
   "%.7lf  %.7lf  %.8lf  %.8lf  %.8lf  " //18
   "%.5lf  %.5lf  %.6lf   %.6lf  %.6lf   %.8lf   %.8lf   %.8lf   %.6lf  %.5lf   %.6lf   %.8lf   %.6lf  %.6lf  %.6lf  "//33
   "%.7lf   %.7lf  %.7lf   %.7lf   %.7lf  %.4lf  %.4lf   %.5lf   %.9lf\n",//42
   los, co.elat[los], co.elong[los], co.b[los], co.l[los], co.dec[los], co.ra[los], nsim, ndet,//9
   log10(s.Nstart),log10(s.Rostart), log10(tei), log10(s.nstart), //13
   log10(d.opta), log10(d.optb), log10(gama), log10(Nevent), Ntot, //18
   d.tE, d.mbase, d.fb, d.Dl, d.Ds, log10(d.piE), log10(d.pirel), log10(d.ros), d.RE, d.vrel, d.u0, log10(d.ml),
   d.fracB,d.fracP, d.fracR, //33
   double(d.Fb*100.0/d.Ndet), double(d.Fp*100.0/d.Ndet), double(d.Fr*100.0/d.Ndet), //36
   double(d.fracS*100.0/d.Ndet), double(d.fracBL*100.0/d.Ndet), d.Nsim, d.Ndet,d.mst,log10(d.emst));//42
   fclose(result);
...
///#############################################################################
double FindB(double Tmag){//checked 6 Feb
   double fblend;  
  
   if(sector==12 or sector==39){
   if(Tmag<6.0) fblend=1.0;  
   else{        fblend=-0.045*Tmag+0.27;  fblend=pow(10.0,fblend);}}
   
   
   if(sector==4 or sector==31){
   if(Tmag<7.0) fblend=1.0;  
   else{        fblend=-0.025*Tmag+0.175;  fblend=pow(10.0,fblend);}}
   
   
   if(sector==13){
   if(Tmag<6.5) fblend=1.0;  
   else{        fblend=-0.03*Tmag+0.195;  fblend=pow(10.0,fblend);}}
   
   if(fblend>1.0 or fblend<0.0){
   cout<<"Error fblend:  "<<fblend<<endl;  int yye;  cin>>yye; }
   return(fblend); 
}   
///#############################################################################
...
{
    int struc,nums, num;
    double rho,rf, Mab[M], Map[M];
    double Ds,Ai[M],Av;
    //double maxnb=0.0;  


    for(int i=0; i<M; ++i){
    s.nsbl[i]=s.Nstart*pow(FWHM[i]*0.5,2)*M_PI/(3600.0*3600.0);
    s.nsbl[i]+= RandN(sqrt(s.nsbl[i]) , 1.5);
    ...
    for(int k=1; k<=s.nsbl[0]; ++k){
    
    do{
    nums=int(RandR(0.0,Num-1.0));
    rho=RandR(0.0,s.Romaxs);
    }while(rho>s.Rostari[nums] or nums<5);
    ...
    if(k<=s.nsbl[i]){s.Fluxb[i]+=fabs(pow(10.0,-0.4*Map[i]));} }
    if(k==1){
    ...
    s.blend[1]=FindB(s.Map[1]); //T-band  blending 
    s.Fluxb[1]=fabs(pow(10.0,-0.4*s.Map[1])/s.blend[1]);//T-band  baseline flux
    s.magb[1] =s.Map[1]+2.5*log10(s.blend[1]);//T-band baseline magnitude
    s.nsbl[1] =double(1.0/s.blend[1]); 

   
   
    for(int i=0; i<1; ++i){//Gaia band
...
///#############################################################################
  ...
  if(l.tE<0.0 or l.tE==0.0 or l.RE<0.0 or  l.Dl>s.Ds  or l.xls>=1.0 or s.ros<0.0 or l.Ml<0.0 or l.t0<0.0){ 
...
///                                                                ///
///XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
void Disk_model(source & s, int counter)
{
   ...
   if(counter%50==0){ 
   ffd=1; 
   sprintf(filename,"./files/light/lcF/%c%c%c%d%c%d.dat",'D','E','N',sector,'_',counter);
